...
<body>
	...
	<c:if test="${sessionScope.id == b.writer || sessionScope.id == 'admin' }">
		<form
			...
			method="post" name="editForm" onsubmit="return check()">
			<h3>글 읽기</h3>
			<table border="1">
				<tr>
					<th>글번호</th>
					<td colspan="2"><input type="text" value="${b.sequence }"
						name="sequence" size="45" readonly><br> 조회수:
						${b.viewcount} ♥${b.recommend_cnt }</td>
				...
				<tr>
					<th>작성 날짜</th>
					<td><input type="text" value="${b.time }" name="time"
						size="45" readonly></td>
				</tr>
				<tr>
					<th>글 제목</th>
					<td><input type="text" value="${b.title}" name="title"
						readonly size="45"></td>
				</tr>
				<tr>
					<th>글 내용</th>
				...
				<tr>
					<td colspan="2"><input type="submit" value="수정"> <input
						type="button" value="삭제" onclick="del(${b.sequence})"></td>
				</tr>
			</table>
			<input type="button" value="목록으로"
				onclick="location.href='${pageContext.request.contextPath}/Board/List_free_designTest.jsp?pagenum=1'">
		</form>
	...
	<c:if test="${sessionScope.id != b.writer }">
		...
		<h3>글 읽기</h3>
		<form action="${pageContext.request.contextPath }/Board/EditBoard.jsp"
			method="post">
			<table border="1" cellspacint="0">
				<tr>
					...
					<td colspan="2"><input type="text" value="${b.sequence }"
						name="sequence" size="45" readonly><br> 조회수:
						${b.viewcount} 좋아요수:${b.recommend_cnt}</td>
				</tr>
				<tr>
					<th>작성자:</th>
					<td><input type="text" value="${b.writer}" name="writer"
				...
				<tr>
					<th>글 제목</th>
					<td><input type="text" value="${b.title}" name="title"
						readonly size="45" ${str }></td>
				</tr>
				<tr>
					<th>글 내용</th>
					<td><textarea rows="15" cols="45" readonly name="content"
							${str }>${b.content}</textarea></td>
				</tr>
			</table>
			<%if(request.getAttribute("like") ==null ){ %><input type="button"
				value="♡" onclick="rec(${b.sequence})">
			<%}
